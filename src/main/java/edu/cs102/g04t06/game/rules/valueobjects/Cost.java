...
public class Cost {
    ...
    public Cost afterBonuses(Map<GemColor, Integer> bonuses) {
        ...
        /**
     * Overloaded bridge method to work with zyik's GemCollection system.
     */
    public GemCollection afterBonuses(GemCollection bonuses) {
        // 1. Call Dong En's logic using the map from your GemCollection
        Cost discountedCost = this.afterBonuses(bonuses.asMap());
        
        // 2. Convert the result back into a GemCollection so ActionExecutor is happy
        return new GemCollection(discountedCost.asMap());
    }
}